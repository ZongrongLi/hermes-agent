...
# ``DELETE /api/sessions/empty`` or ``POST /api/sessions/bulk-delete``
# as "operate on the session with id 'empty'" / "'bulk-delete'", which
# would 404 (or worse, succeed and delete the wrong row). Same story as
# the older ``/api/sessions/search`` endpoint up at line ~1191. If you
# split or reorder this block, move every route in it together.
